...
matchOut <- matchit(aiusebinary ~ curr_gpa + y1_gpa + anxiety_rate + age + ai_strict_rate + tech_prof_acad_rate + tech_prof_rate + aisub_time, data = survey, method = "nearest")
...
```{r}
lmtest <- lm(curr_gpa ~ factor(major) + ai_use_freq_rate + ai_impact_rate + ai_strict_rate, data = survey_test)
summary(lmtest)
```