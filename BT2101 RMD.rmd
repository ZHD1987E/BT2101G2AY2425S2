...
```{r}
lmtest <- lm(patent_count ~ net_elec_imports + renewables_electricity + population + gdp + factor(year) + factor(Entity), combine)
summary(lmtest)
```
...
```{r}
plot1 <- ggplot(combine, aes(patent_count, renewables_electricity, colour = Entity)) + geom_point()
plot1
```
