...
matchOut <- matchit(aiusebinary ~ y1_gpa + time_exam + time_non_educ + ethic_rate + procastinate_rate + effaciacy_rate + ability_rate + ainoneduc_rate + ai_use_nounderstand + anxiety_rate + age + ai_strict_rate + tech_comfort_rate + tech_prof_rate + aisub_time, data = survey, method = "nearest")
...
```{r}
lmtest <- lm(curr_gpa ~ ai_use_freq_rate + y1_gpa + factor(major) + factor(time_exam) + factor(learn_style), data = survey_test)
summary(lmtest)
```
